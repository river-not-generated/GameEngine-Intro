...
namespace nu
{
	...
	Rect Tilemap::GetTileRect(const Layer& layer, int tileId)
	{
		...
		if (m_tileWidth == 0 || m_tileHeight == 0) return Rect();

		Vector2 size = layer.texture->GetSize();
		if (size.x == 0 || size.y == 0) return Rect();
		// get the number of tiles in a row (texture size / tile width size)
		int columns = (int)(size.x / m_tileWidth);

		// get the column / row of the tile
		int c = (tileId - 1) % columns;
		int r = (tileId - 1) / columns;

		// return source rectangle of tile in texture
		return Rect
		{
			(float)(c * m_tileWidth),
			(float)(r * m_tileHeight),
			(float)(m_tileWidth),
			(float)(m_tileHeight)
		};
	}
